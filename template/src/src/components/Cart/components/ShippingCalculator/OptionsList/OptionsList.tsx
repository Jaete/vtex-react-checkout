...
function OptionsList({ options, selectedShipping, onSelect }: OptionsListProps) {
  return (
    <div className={styles.options}>
      ...
      <div className={styles.optionsList}>
        {options.map((option, index) => {
          ...
          const displayName = pickup ? option.pickupAddress ?? option.pickupStoreName ?? option.name : option.name;
          return (
            <ShippingOption
              key={option.id}
              index={index}
              ...
              pickupStore={pickup && option.pickupAddress ? option.pickupStoreName : undefined}
            />
          );
        })}
      </div>
